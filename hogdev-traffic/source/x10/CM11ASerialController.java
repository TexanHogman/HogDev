...
public class CM11ASerialController implements Runnable, Controller
{
	...
	public CM11ASerialController(String comport) throws IOException
	{
		...
		{
			throw new IOException("Unsupported comm operation: "
					+ ucoe.getMessage());
		}
	...
	public void addCommand(Command command)
	{
		...
		{
			// my hack to block until request is complete
			try
			{
				Thread.sleep(2000);
			}
			...
			// if I have done this for 30 sec throw runtime exception to allow caller to yank the carpet and start over
			if(++i >= 15)
			{
				commandInitiated = false;
				queue.empty();
				throw new WastedException();
			}
		}
...
//	}
	...
	private synchronized void initiateCommand(Command nextCommand)
	{
		if (nextCommand != null)
		{
			try
	...
	private synchronized void handleChecksum(byte checksum)
	{
		...
		if (nextCommand != null)
		{
			...
			{
				LogHandler.logMessage("CheckSum: "
						+ Integer.toHexString(checksum), 2);
			}
		}
