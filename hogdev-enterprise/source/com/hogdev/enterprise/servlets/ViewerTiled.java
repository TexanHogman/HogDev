 ...
 * @author
 */
public class ViewerTiled extends BaseRelative
{
	/**
     * 
     */
    private static final long serialVersionUID = 1996532937642948670L;
    static Logger logger = Logger.getLogger(ViewerTiled.class);
		...
		throws ServletException, IOException
	{
		...
		try
		{
			...
			if (resource.getType() == Constants.DIR)
			{
				...
				switch (iView)
				{
					...
					case Constants.DETAILS :
						request.getRequestDispatcher("/details.do").forward(
							request,
							response);
						break;
					case Constants.LIST :
					default :
						request.getRequestDispatcher("/list.do").forward(
							request,
							response);
						break;
				}
			...
			else
			{
				...
				request.getRequestDispatcher("/walker.do").forward(
					request,
					response);
				return;
			}
